...
public class UserManageDAO {
  ...
  public void addUser(String owner_id, String pw, String tel, String name, String addr) throws SQLException {
    ...
    try {
      conn = dbconn.getConnection();
      String insertUser = "INSERT INTO owner(owner_id, pw, tel, name, addr, delete_flag) VALUES (?, ?, ?, ?, ?, 'x')";
  ...
  public List<UserInfoVO> selectAllUser() throws SQLException {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        uiVO = new UserInfoVO(rs.getString("owner_id"), rs.getString("name"), rs.getString("tel"), rs.getString("addr"),
            new ArrayList<RegisteredCarVO>(Arrays.asList(rcVO)));
        user.add(uiVO);
      }
    } finally {
      dbConn.dbClose(conn, pstmt, rs);
