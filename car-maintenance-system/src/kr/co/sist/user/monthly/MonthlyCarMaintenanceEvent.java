...
public class MonthlyCarMaintenanceEvent extends MouseAdapter implements ActionListener {
  ...
  private void updateTableData() {
    ...
    try {
      ...
      for (MonthlyCarMaintenanceVO mcmVO : maintenanceData) {
        ...
        System.out.println(
            "Adding row: OwnerName=" + mcmVO.getOwnerName() + ", MaintenanceDate=" + mcmVO.getMaintenanceDate2());
        // 테이블에 각 행 추가
  ...
  public void mouseClicked(MouseEvent e) {
    if (e.getSource() == monthlyCarMaintenanceView.getJtabMonthlyCarMaintenanceData()) {
      ...
      if (selectedRow != -1) {
        String ownerName =
            (String) monthlyCarMaintenanceView.getJtabMonthlyCarMaintenanceData().getValueAt(selectedRow, 0);
        String maintenanceDate =
            (String) monthlyCarMaintenanceView.getJtabMonthlyCarMaintenanceData().getValueAt(selectedRow, 1);

        // 디버깅 코드
        System.out.println("Selected Row Data - Owner Name: " + ownerName + ", Maintenance Date: " + maintenanceDate);
