...
public class MonthlyCarMaintenanceDAO {
  ...
  static public List<MonthlyCarMaintenanceVO> getMaintenanceData(String selectedItem) throws SQLException {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        maintenanceData.add(new MonthlyCarMaintenanceVO(rs.getString("ownerName"), rs.getString("maintenanceDate")));
      }
    } finally {
      // 연결 종료
      dbConn.dbClose(conn, pstmt, rs);
  ...
  public static List<MonthlyCarMaintenanceVO> getDetailedData(String ownerName, String maintenanceDate)
    ...
    try {
      // 쿼리문 생성
      String query =
          "SELECT " + "inventory.part_name, " + "inventory.part_price, " + "car_maintenance_settlement.part_cnt, "
              + "(inventory.part_price * car_maintenance_settlement.part_cnt) AS total_price " + "FROM " + "owner "
              + "INNER JOIN " + "reserved_car ON owner.owner_id = reserved_car.owner_id " + "INNER JOIN "
              + "car_maintenance_settlement ON reserved_car.car_id = car_maintenance_settlement.car_id " + "INNER JOIN "
              + "product_cnt ON car_maintenance_settlement.car_maintenance_id = product_cnt.car_maintenance_id "
